...
$$
\hat \beta=\mathbf{(H^TH)^{-1}H^Ty}\tag{8.2}
$$
...
$\hat\beta$的协方差矩阵估计为
$$
\widehat{Var}(\hat\beta)=\mathbf{(H^TH)}^{-1}\hat\sigma^2\tag{8.3}
$$
...
$$
\widehat{se}[\hat\mu(x)]=[h(x)^T\mathbf{(H^TH)^{-1}}h(x)]^{\frac{1}{2}}\hat{\sigma}\tag{8.4}
$$
![](../img/08/fig8.2.png)
...
\mu(X)&=\sum\limits_{j=1}^7\beta_jh_j(x).\qquad\qquad\qquad\tag{8.5}
\end{align}
$$
