...
!!! note "weiya 注：翻译疑问"
	“The response vector y is the vector of $2^p$ counts in each of the cells of the multiway tabulation of the data.” 这里我理解为 $2^p$ 维，因为有 $2^p$ 个单元，每个单元是指代观测点的个数，但这样似乎与 [练习 17.12](https://github.com/szcf-weiya/ESL-CN/issues/141) 矛盾．
...
- **迭代比例过滤 (Iterative proportional fitting, IPF)** 在梯度式 \eqref{17.34} 中采用柱坐标系．每一步更新一个参数使得梯度等式正好为 0．这在柱坐标系中进行直到所有梯度为 0．一个完整的周期与梯度赋值花费同样的计算量，但是可能更有效．Jirouśek and Přeučil (1995)[^4] 采用 junction trees 实现了 IPF 的一个有效版本．
