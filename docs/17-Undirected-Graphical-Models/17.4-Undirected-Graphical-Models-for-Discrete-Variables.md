...
!!! note "原书脚注："
	每个单元计数看成是单独的独立泊松变量。通过在总数 $N$ 的条件下得到对应 (17.28) 的多项式模型（在这个框架下也是泊松分布）。

!!! note "weiya 注：翻译疑问"
	“The response vector y is the vector of $2^p$ counts in each of the cells of the multiway tabulation of the data.” 这里我理解为 $2^p$ 维，但这样似乎与 [练习17.12](https://github.com/szcf-weiya/ESL-CN/issues/141) 矛盾。
