...
其中 $\gamma_i\ge 0,\sum_{i=1}^\infty \gamma_i^2 < \infty$。${\cal H}_K$ 的元素是关于这些 **特征函数 (eigen-functions)** 的展开，即
...
!!! note "weiya 注：Induced Norm"
    希尔伯特空间是完备内积空间。这里 **导出范数 (Induced Norm)** 是通过内积定义的，即
    $$
    \Vert f\Vert_{{\cal H}_K} := \sqrt{\langle f,f\rangle_{{\cal H}_K}}.
    $$
