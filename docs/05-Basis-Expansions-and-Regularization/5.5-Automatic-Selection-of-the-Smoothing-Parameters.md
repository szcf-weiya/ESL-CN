...
选择回归样条结点的位置及数目是一项复杂的组合任务，除非强加一些简化。第 9 章的 MARS 过程采用贪婪算法以及额外的近似来达到实际的妥协。这里我们不会深入讨论。
...
$$
\begin{align}
\CV(\hat f_\lambda) &=\frac 1N\sum_\limits_{i=1}^N(y_i-\hat f_\lambda^{(-i)}(x_i))^2\qquad (5.26)\\
