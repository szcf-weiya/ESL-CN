...
- `k`: degree，则 $M$ = `k` + 1
- `t`: 长度为 `n+k+1` 的结点序列，即 `n` + M, 其中定义域为 `t[k] .. t[n]`，这两个也刚好是边界结点，而 `t[0], t[1], ..., t[k-1]` 即对应增广的结点序列 $\tau_1,\ldots, \tau_{M-1}$，以及 `t[n+1], t[n+2], ..., t[n+k]` 对应 $\tau_{K+M+2},\ldots, \tau_{K+2M}$. 而且需注意到边界结点不再出现在增广结点序列中，但一般 $\tau_{M} = \xi_0; \tau_{K+M+1} = \xi_{K+1}$, 即 `t[k], t[k+1], ..., t[n]` 为基础结点（包含两个边界结点），$\xi_0,\xi_1,\ldots,\xi_K, \xi_{K+1}$，个数为 `n - k + 1` = K + M - `k` + 1 = K + 2
