...
假设${\cal A}_k$是第$k$步开始时的变量活跃集，$\beta_{{\cal A}_k}$为这一步中变量的系数向量；其中有$k-1$个非零值，刚刚进入的变量系数值为0.如果当前残差为$\mathbf{r}_k=\mathbf{y}-\mathbf{X}_{{\cal A}_k}\beta_{{\cal A}_k}$,则当前步的方向为
...
对于LAR和lasso，会发生很奇怪的事情。这些技巧在光滑的方面比最优子集选择更加自适应，因此估计自由度会更加地难以驾驭。特别地，可以证明经过$k$步