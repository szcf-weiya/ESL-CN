...
这可以由 **多变量高斯定理** 自然得出。这里 $f(x)$ 为向量函数 $(f_1(x),\ldots,f_K(x))$，而且 $y_i$ 为观测值 $i$ 的 $K$ 维响应向量。然而，可以证明解也是由式 \eqref{3.39} 给出；$K$ 个分离的回归忽略了相关性（[练习3.11](https://github.com/szcf-weiya/ESL-CN/issues/73)）。如果 $\Sigma_i$ 不同观测为不同值，则不再是这种情形，而且关于 $\mathbf{B}$ 的解不再退化(decouple).
